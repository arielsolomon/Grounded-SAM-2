...
BOX_THRESHOLD = [0.1,0.15,0.2,0.25,0.3,0.35,0.4]
TEXT_THRESHOLD = [0.1,0.15,0.2,0.25,0.3,0.35,0.4]
...
for bt in BOX_THRESHOLD:
    for tt in TEXT_THRESHOLD:
        IMG_NAME = Path(IMG_PATH).stem+str(bt)+'_'+str(tt)
        ...
        for crop_idx, (x1, y1, x2, y2) in tqdm(enumerate(crops), total=len(crops), desc="Processing crops"):
            #print(f"Processing crop {crop_idx + 1}/{len(crops)}: ({x1}, {y1}, {x2}, {y2})")
            
            # Extract crop from original image
            crop_image = original_cv_image[y1:y2, x1:x2]
            crop_h, crop_w, _ = crop_image.shape
            
            # Save crop as temporary image file
            ...
            try:
                ...
                # Predict on crop (without autocast for Grounding DINO)
                ...
                if len(boxes) > 0:
                    ...
                    boxes = boxes * torch.Tensor([crop_w, crop_h, crop_w, crop_h])
        ...
        print(f"Saved YOLO labels to {os.path.join(OUTPUT_DIR, f'{IMG_NAME}.txt')}")
        ...
        if filtered_detections:
            ...
            cv2.imwrite(os.path.join(OUTPUT_DIR, f"{IMG_NAME}_tiled_detections.jpg"), annotated_frame)
            print(f"Saved annotated image to {os.path.join(OUTPUT_DIR, f'{IMG_NAME}_tiled_detections.jpg')}")
